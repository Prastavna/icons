...
import { above, aboveFilled, below, belowFilled } from "../../src/icons/ties/index.ts";

const outlineIcons = [
  { icon: above, name: "tie-above" },
  { icon: below, name: "tie-below" },
];

const filledIcons = [
  { icon: aboveFilled, name: "tie-above-filled" },
  { icon: belowFilled, name: "tie-below-filled" },
];

describe("tie icons — outline", () => {
  test.each(outlineIcons)("$name has correct metadata", ({ icon, name }) => {
    expect(icon.iconName).toBe(name);
    expect(icon.category).toBe("tie");
